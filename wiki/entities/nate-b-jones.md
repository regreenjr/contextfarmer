...
## New in [[youtube-digest-apify-2026-05-28]]

**Two more frameworks (22nd + 23rd)** — extends his cadence to **23 named frameworks in 24 days** (T/C/L/D 2026-05-04 → [[document-truth-layer]] 2026-05-27). After the 2026-05-26 interview-format experiment ([[platform-agent-asymmetry]]), he reverts to solo-monologue framework videos. Both new frameworks are **trust/reliability** frameworks.

### #2 [[public-ai-work]] — apprenticeship gap / Polanyi's paradox (org-learning framework)

His **22nd framework** — *Shopify CEO Reveals Their Secret AI Developer* (18.8K views, 2026-05-26, 16:24). The **organizational-learning** framework: AI adoption is *not* a tooling problem solved by buying licenses — your most valuable AI work is **invisible** (done in private DMs), and that invisibility widens an **apprenticeship gap**.

**Unlock event**: **Shopify's "River" agent runs only in public Slack channels** (deliberately watchable). First Shopify-internal-agent reference in the vault.

**Core claims**:
- **Private AI chats widen an apprenticeship gap** (chapter 05:46) — juniors can't watch seniors use AI, so judgment never transmits.
- **Polanyi's paradox** (chapter 07:30) — "we know more than we can tell"; tacit AI-use knowledge transmits by observation, not by a prompt library (chapter 11:00).
- **Tooling choices are frontier choices** (chapter 04:45) — Slack (public-channel-native) vs Teams/Copilot (DM-native) determines whether AI work is observable. Extends the [[agent-substrate]] thesis to the collaboration-tool layer.
- **Declared spaces + senior people** (chapter 15:00) — regulated teams expose work safely via declared channels; the unlock is senior people willing to run real work where everyone can watch.

→ New concept: [[public-ai-work]]. Org-design-side complement to T/C/L/D (worker-side). Reframes [[context-farming]] / [[karpathy-llm-wiki]] as organizational apprenticeship infrastructure.

### #1 [[document-truth-layer]] — "a prompt asks for output, a workflow defines trust" (document-reliability framework)

His **23rd framework** — *I Built a Deck With AI, Then Made a Second AI Attack It.* (16.8K views, 2026-05-27, 19:29). The **document-reliability discipline** for building Office files (PowerPoint/Excel/Word) with AI agents at the center.

**Core thesis**: *"a prompt asks for output, but a workflow defines trust"* — a clean-looking deck with an undefendable number is **worse than no deck at all**.

**The four stages** (chapter 05:25): **sources → structure → creation → verification**. Trust is built in stage 4, not stage 3.

**The hostile reviewer prompt** (the title's "second AI attack it") — a **separate adversarial AI pass** whose only job is to find the undefendable claim; *"what a hostile reviewer catches that proofreading never will."* The **deliverable-boundary instance** of [[agent-security]]'s LLM-as-judge (action-boundary) pattern.

**The task risk gradient** (chapter 10:07) — where AI is highest vs lowest risk on a document task. Highest: blended actuals + plan data as one number (the board-deck failure, chapter 07:00). Lowest: structure/formatting/first-draft prose. **Models guess without sources** (chapter 08:40) — goal-oriented models fabricate a plausible number to "finish"; the truth layer makes that structurally impossible.

→ New concept: [[document-truth-layer]]. Document-creation-side complement to [[project-room-workflow]] (canvas-before-prompt for writing). **⚠️ Naming collision**: distinct from [[prove-it-economy]]'s marketing-side "truth layer" — same brand term, different surface; flagged on both pages.

## New in [[youtube-digest-apify-2026-05-29]]

### #1 [[agent-analytics]] — product analytics for AI agents (run-not-click framework)

His **24th framework** — *A Cursor Agent Wiped a Database in 9 Seconds. Agent Analytics Would Have Seen It Coming.* (14.3K views, 2026-05-28, 11:51). The **product-analytics-side** framework: *"product analytics for AI agents has to start from the run, not the click."*

**The framing claim**: *"the common story is that agent failures are engineering incidents, but the reality is that most of them are product analytics failures hiding inside the agent run."*

**Unlock event**: a **Cursor agent deleted a database in nine seconds** (chapter 01:34) — reads as an engineering incident, but the deeper failure is that no product-analytics layer existed to see the run heading toward an irreversible action. Joins McKinsey-Lilly + Mozilla-271 + Sullivan-Cromwell + Shopify-River as a named-failure unlock event. **"Ten billion tokens of agent code in production"** (00:46) is the scale claim.

**Core claims**:
- **The agent run replaces the session** as the unit of product behavior (03:09) — delegated work is the new measured primitive.
- **Chat logs are not product analytics** (04:08) and **engineering traces are not product analytics** (05:02) — both are mistaken for it, which is why dashboards miss the failure (02:25).
- **The completion-vs-acceptance gap** (08:21) measures trust — an agent can complete a run the user rejects.
- **The correction is your most valuable signal** (07:01) — user edits/undos/overrides are the highest-information event; instrument first.
- **Salesforce Agent Work Units** (05:59) name the delegated work — the same unit [[agent-metering]] sees from the *pricing* side, here from the *analytics* side.
- **Ship three events first** (09:42) — minimum instrumentation set (specifics gated to transcript / Substack: *"Full Post w/ Prompts"*).

**Why it matters**: the **fourth distinct boundary framework** — after [[agent-security]] (action boundary), [[long-running-benchmarks]] (eval boundary), and [[infrastructure-control-layer]] (infra observability), Agent Analytics names the **run / product boundary**. Its key move is insisting the *product* layer (acceptance, correction, trust) is distinct from the *engineering* layer (traces) — the same distinction [[agent-metering]] draws between work-units and tokens. Lands the same day as [[nate-herk]]'s [[opus-4-8]] "honesty upgrade", making 2026-05-29 a **trust/reliability batch** (run-side analytics + model-side honesty).

→ New concept: [[agent-analytics]]. Updates: [[agent-security]] (run-boundary complement to the action-boundary judge), [[agent-metering]] (Agent Work Units now seen from the analytics side).

## New in [[youtube-digest-apify-2026-05-30]]

### #3 [[software-abundance-pm]] — the PM job after software gets cheap (role-redefinition framework)

His **25th framework** — *Cheap software made your PM job harder, not easier. Here's the new job.* (11.4K views, 2026-05-29, 12:38). The **product-management role redefinition** for the cheap-software era — his first framework aimed at the **PM function** specifically.

**The framing claim**: *"the common story is that PMs just become prototypers — but the reality is more complicated. Prototyping is table stakes. The real shift is that AI moved the bottleneck, and product management becomes the discipline that **classifies software abundance into market value**."*

**The central inversion — the artifact arrives before the request**: when building is cheap, someone (or an agent) has *already built* the thing. The old PM gate (*"should I build this?"*) flips to *"somebody already built something — should the company actually rely on it?"* The bottleneck moved from building to **governing what got built**.

**Chapter map**: PM in the age of AI (0:00) → **Microsoft's million-asset reality check** (1:14) → **Why the non-technical PM is finished** (2:30) → Where the old PM filter breaks down (3:45) → Broad building meets governance risk (5:20) → **Market judgment is the new scarce thing** (6:40).

**Core claims**:
- **Microsoft already has 1M+ internal Power Platform assets** (1:14) — the named reality check; software abundance is already here inside large orgs. The **prototype commons** creates hidden demand *and* hidden risk.
- **The non-technical PM is finished** (2:30) — a PM who can neither build nor govern has no filter left; prototyping is table stakes.
- **A production class ladder** — a tiered reliance model for cheap software where **demotion matters as much as promotion** (moving an asset *down* — deprecate / sandbox / stop relying — is a first-class PM act). Tiers gated to Substack.
- **Market judgment is the new scarce thing** (6:40) — when building is cheap, the durable skill is classifying which abundance creates value.

**Why it matters**: the **consumer-of-abundance mirror** of his [[work-primitive]] (producer-of-action). Where [[capital-allocation-framework]] asks *should we build/buy this workflow* (pre-build), software-abundance-PM asks *given it's already built, should we depend on it* (post-build governance) — the two bracket the build decision. Continues his "judgment is the scarce thing" thesis ([[long-running-benchmarks]], [[agent-analytics]]). Substack monetization (*"Full Post w/ Prompts"*) gates the production-class-ladder spec. Extends his cadence to **25 frameworks in 26 days**.

→ New concept: [[software-abundance-pm]]. Updates: [[capital-allocation-framework]] (post-build governance complement), [[ai-consulting]] (a production-class-ladder governance audit is a 3Ps deliverable).

## New in [[youtube-digest-apify-2026-06-02]]

**Two videos — his 26th framework + a practice-update.** The framework video is his first dedicated **career-evidence / hiring-side** framework; the practice video consolidates his existing workflow frameworks into "here's how I actually work now."

### #1 [[portable-judgment]] — career evidence is judgment, not output (26th framework)

His **26th framework** — *Microsoft Says 86% Treat AI Output as a Starting Point. Your Resume Just Stopped Working.* (23.1K views, 2026-05-31, 10:34). The **career-evidence** framework for the AI age.

**The framing claim**: *"the common story is that AI makes us more productive — but the reality is that AI makes **everyone look productive**, and the old career evidence is **losing signal**."*

**The unlock stat**: **Microsoft says 86% of workers treat AI output as a starting point** (not a finished product). If nearly everyone edits AI drafts into polished artifacts, then a polished artifact no longer proves the person *understood* the work — *"your resume just stopped working."*

**Chapter map**: The stat that changes career evidence (0:00) → **Why the AI age is the age of whiteboards** (1:20) → The old evidence stopped carrying signal (2:45) → Why standard portfolio advice falls short (3:50) → **Four questions that make judgment portable** (5:10) → How whiteboard sessions reveal how you learn (6:38) → **Comprehension over generation** (7:30) → Starting strong in a new role (8:10) → Making reasoning visible without a physical board (9:25) → The evidence people actually need now (9:55).

**Core claims**:
- **The age of whiteboards** (1:20) — live reasoning under pressure reveals real judgment that AI output can't fake. Whiteboard conversations, not portfolios, are the evidence.
- **Four questions that make your thinking portable and shareable** (5:10) — convert tacit in-the-moment judgment into evidence that *travels* beyond a single role (the four questions gated to his Substack + TalentBoard link).
- **Comprehension over generation** (7:30) — the durable signal is whether you comprehend the work, not whether you can generate an artifact (generation is commoditized).
- **Preserve the evidence before your badge stops working** (9:25–9:55) — capture your reasoning artifacts before you lose role access; make reasoning visible without a physical board.

**Why it matters**: the **worker/career-evidence extension** of [[prove-it-economy]]'s "prove-it economy for individuals" — prove-it-economy named verifiable artifacts at the *brand* layer (agents interpret you); Portable Judgment names visible-reasoning evidence at the *career/hiring* layer (humans interpret you). It's the *how-you-prove-it* complement to **T/C/L/D**'s *what-work-is-durable* (his 1st framework), and a sibling to [[public-ai-work]] (both "make invisible reasoning visible"). Links his **TalentBoard** product. Continues his "judgment is the scarce skill" thesis. Extends his cadence to **26 frameworks in 28 days** (T/C/L/D 2026-05-04 → portable-judgment 2026-05-31).

→ New concept: [[portable-judgment]]. Updates: [[prove-it-economy]] (career-evidence extension), [[public-ai-work]] (sibling visible-reasoning framework).

### #2 *My AI Workflow Has Changed* — practice-update (not a numbered framework)

**49.2K views, 2026-05-30, 5:39 — highest-view video in the batch.** A **practice-update** on his own mid-2026 working pattern after pushing [[codex]] + [[claude-code]] to their limits — *not* a new named framework, but a consolidation of existing ones.

Framing: *"the common story is that prompt engineering is still the skill that separates beginners from experts — but the reality in mid-2026 is more nuanced... the way serious AI users work has shifted dramatically in just the last few weeks."*

**Core moves**: build **local file folders as context windows** (0:15 — files-as-canvas, same as [[project-room-workflow]] + [[karpathy-llm-wiki]]); [[codex]] handles **50,000-word documents** other tools can't (1:18); **prompting has shifted** (2:23) → **shape the task before execution** (3:13 — [[ai-question-method]]'s Principle 2 *ask what good looks like* as a practice habit); **multi-threaded drafting and review** (4:26); **why not to pick a side** (5:13 — use both substrates, confirming the [[free-sample-phase]] "use both at once" play + [[claude-code-vs-codex]] vendor-agnostic stance).

**Why it matters**: notably, this process video (49.2K) *outperformed* his framework video (#1, 23.1K) in the same batch — unusual for him (his pattern is framework content winning). The thesis — **the move from giving AI instructions to shaping work alongside it** — ties his own habits back to [[ai-question-method]] + [[project-room-workflow]]. Counted as a practice synthesis, not a 27th framework.

→ Updates: [[ai-question-method]] ("shape the task before execution" as practice), [[project-room-workflow]] (local folders as context windows), [[codex]] (50K-word doc datapoint), [[free-sample-phase]] / [[claude-code-vs-codex]] ("why not to pick a side").

## New in [[youtube-digest-apify-2026-06-05]]

### #1 [[harness-over-model]] — your workflow doesn't care about the score (27th framework)

His **27th framework** — *Opus 4.8 Scored 81. Your Workflow Doesn't Care.* (34.3K views, 2026-06-03, 26:36). The **model-selection counterpart** to his [[long-running-benchmarks]] harness thesis, and the explicit **skeptic's read on [[opus-4-8]]** against [[nate-herk]]'s enthusiastic adoption coverage — the vault's **first tracked creator disagreement on a Claude model.**

**The framing claim**: *"the common story is that a stronger benchmark score automatically makes a model your daily driver — but the reality is that harnesses, compute, and workflow reliability now matter just as much as raw model intelligence."* [[opus-4-8]] is a **checkpoint release**: *"the product harness around the model now matters more than the model itself."*

**Core claims**:
- **Reasoning effort became unpredictable on 4.8** — the effort control [[nate-herk]] treats as a clean lever is, in Jones's testing, inconsistent (same setting, different behavior).
- **The effort-level trap** — **Vending-Bench data shows `max` effort can make long-running work *worse***. A second long-running benchmark enters the vault; the effort knob is unreliable *and* non-monotonic.
...
- **The /workflows command reveals agent design** — [[dynamic-workflows]] is a window into the harness, which is where agent architecture lives.
- **Architect for harness flexibility** — *"builders and engineering leaders who architect for harness flexibility now will avoid the budget [trap]"* — design for swappable harnesses, not a permanent model choice ([[free-sample-phase]] portability instinct).

**Why it matters**: extends his standing harness thesis from **evaluation** ([[long-running-benchmarks]]: the harness is what you measure) into **model-selection** (the harness is what you optimize and select on). Lands directly against [[nate-herk]]'s *Opus 4.8 Just Dropped* (101K) — Herk = "here's how to use the new knobs," Jones = "the knobs are unreliable and the harness is what moves outcomes." The effort-level trap is a directly billable client insight; the routing guide is the same consulting-artifact shape as his [[agent-metering]] four-question diagnostic. Extends his cadence to **27 frameworks in ~30 days** (T/C/L/D 2026-05-04 → harness-over-model 2026-06-03).

→ New concept: [[harness-over-model]]. Updates: [[opus-4-8]] (checkpoint-release reframe + effort-level instability), [[long-running-benchmarks]] (Vending-Bench effort trap; model-selection extension), [[codex]] (harness-beat-the-score + routing guide), [[dynamic-workflows]] (/workflows reveals agent design).
...
- [[youtube-digest-apify-2026-05-28]] #2 *Shopify CEO Reveals Their Secret AI Developer* (18.8K, 2026-05-26) — [[public-ai-work]] 22nd framework (Shopify River agent runs only in public Slack + apprenticeship gap + Polanyi's paradox + tooling-choices-are-frontier-choices + declared spaces + senior people willing to work in public)
- [[youtube-digest-apify-2026-05-28]] #1 *I Built a Deck With AI, Then Made a Second AI Attack It.* (16.8K, 2026-05-27) — [[document-truth-layer]] 23rd framework ("a prompt asks for output, a workflow defines trust" + four-stage sources→structure→creation→verification pipeline + hostile-reviewer prompt + task risk gradient + models-guess-without-sources)
- [[youtube-digest-apify-2026-05-29]] #1 *A Cursor Agent Wiped a Database in 9 Seconds. Agent Analytics Would Have Seen It Coming.* (14.3K, 2026-05-28) — [[agent-analytics]] 24th framework ("product analytics is the rudder on your agents" + the agent run replaces the session as the unit of product behavior + chat-logs-and-engineering-traces-are-not-product-analytics + completion-vs-acceptance-gap + the-correction-is-your-most-valuable-signal + Salesforce Agent Work Units + three-events-to-ship-first + Cursor-DB-wipe unlock)
- [[youtube-digest-apify-2026-05-30]] #3 *Cheap software made your PM job harder, not easier. Here's the new job.* (11.4K, 2026-05-29) — [[software-abundance-pm]] 25th framework (artifact-arrives-before-the-request + classifies-software-abundance-into-market-value + non-technical-PM-is-finished + Microsoft 1M+ Power Platform assets + production-class-ladder where demotion-matters-as-much-as-promotion + market-judgment-is-the-new-scarce-thing)
- [[youtube-digest-apify-2026-06-02]] #1 *Microsoft Says 86% Treat AI Output as a Starting Point. Your Resume Just Stopped Working.* (23.1K, 2026-05-31) — [[portable-judgment]] 26th framework (86%-treat-AI-output-as-starting-point + the-age-of-whiteboards + four-questions-that-make-judgment-portable + comprehension-over-generation + preserve-the-evidence-before-your-badge-stops-working + TalentBoard)
- [[youtube-digest-apify-2026-06-02]] #2 *My AI Workflow Has Changed (Here is What I Learned)* (49.2K, 2026-05-30) — **practice-update, not a numbered framework** (local folders as context windows + Codex 50K-word docs + "shape the task before execution" + multi-threaded drafting/review + "why not to pick a side")
- [[youtube-digest-apify-2026-06-05]] #1 *Opus 4.8 Scored 81. Your Workflow Doesn't Care.* (34.3K, 2026-06-03) — [[harness-over-model]] 27th framework (Opus 4.8 = checkpoint release + harness > benchmark score + reasoning-effort unpredictable + Vending-Bench effort-level trap + Codex harness beat the score + routing guide)
- [[youtube-digest-apify-2026-06-06]] #3 *My Codex Ran 800 Million Tokens in A Day. The Real Story Isn't Cost.* (14.5K, 2026-06-05) — [[token-burn-dashboard]] 28th framework (token-burn-tracks-with-smarter-results + the-point-is-the-feedback-loop + built-in-Codex + multi-agent-runs-reveal-real-habits + assistant-work-vs-computer-work + ranking-by-volume-backfires + Tufte-viz + 800M-tokens)

## New in [[youtube-digest-apify-2026-06-06]]

### #3 [[token-burn-dashboard]] — the feedback loop, not the cost (operator self-instrumentation framework)

His **28th framework** — *My Codex Ran 800 Million Tokens in A Day. The Real Story Isn't Cost.* (14.5K views, 2026-06-05, 21:05). He instrumented his own AI usage with a **token-burn dashboard** built in [[codex]] and burned ~**800 million tokens in a single day** — and the point was never the number.

**Core claims**:

1. **Token burn tracks with smarter AI results** — burn is a proxy for engagement depth (more iteration / agents / context), not a cost leak; *"the common story is that burning more tokens is just waste — but the reality is more complicated."*
...
4. **Multi-agent runs reveal your real habits** — fan-out usage exposes what you actually do with AI vs what you think you do.
...
**Tooling**: open-source **Tufte visualization skill** + **logarithmic scaling**; guide + build prompt + kit gated to his Substack.

**Strategic significance**:

1. **The operator-side cousin of [[agent-analytics]]** (his 24th framework) — agent-analytics measures the agents you *ship*; the token-burn dashboard measures *your own* AI behavior. Same "instrument the run, not the vibe" instinct turned inward (*token burn is the rudder on you*).
2. **Empirical support for [[harness-over-model]]** at the individual scale — *"stuck on the wrong side has nothing to do with the model"* + *"burn tracks with smarter results"* both restate the harness thesis; built in [[codex]], consistent with his standing "the Codex harness outperformed raw model intelligence" finding.
3. **Sits beside the same-week [[when-ai-builds-itself]]** ([[nate-herk]] #2) — Anthropic's 80%+-AI-written-code disclosure is the org-scale version of the same recursive-leverage story the dashboard measures at the individual scale.
4. **Substack monetization pattern continues** — *"Token Burn Dashboard Guide: natesnewsletter.substack.com/..."* gates the build prompt + ready-made kit + the alternative-to-volume leadership metric.

→ New concept: [[token-burn-dashboard]]. Updates: [[codex]] (800M-token day + dashboard built in Codex), [[agent-analytics]] (operator-side cousin).

## New in [[youtube-digest-apify-2026-06-09]]

### #2 [[ai-layoffs]] — a layoff is a strategy signal (labor-market framework)

His **29th framework** — *Beyond The Hype: Why Meta And Block Are Firing People* (15.3K views, 2026-06-08, 20:18). His **labor-market / org-design** framework for **reading the 2026 layoff wave as strategy signals** rather than as proof "AI replaced the workers."

**The framing claim** (his signature shape): *"The common story is that AI made these workers redundant, but the reality is far messier and far more useful to understand."* Thesis line: **a layoff is the loudest strategy signal a company can send — read it.** Core principle: **no two AI layoffs mean the same thing.**

**The four reads**:

1. **Hyperscaler reallocation** — **Meta cuts staff while burning GPU billions.** A layoff at a company *simultaneously* spending record CapEx is a *reallocation* signal (headcount → GPU/datacenter), not distress. Ties directly to [[ai-supply-contract]]'s capacity-constrained CapEx surface.
2. **Strategy-encoding** — **Jack Dorsey's Block layoffs show a real AI vision.** Some cuts disclose a coherent AI-native restructuring rather than hiding weakness.
3. **Distress tell** — **activity-based layoffs reveal hidden distress.** Cutting by *activity/function* rather than performance can betray balance-sheet distress the company isn't disclosing. Read the cut's *shape* to read the company's hidden health.
4. **Job-seeker map** — where to run vs where to look; *"for leaders and job seekers alike, a layoff is the loudest strategy signal a company can send."*

**Strategic significance**:

1. **Completes the worker→career→labor-market arc** — T/C/L/D (which of *my tasks* survives) + [[portable-judgment]] (how *I* prove judgment) + **AI Layoffs** (which *employer* is at risk). The same disruption read from the inside (your work) and the outside (the company's cuts). Inverts [[capital-allocation-framework]] — that's the company's hire/automate decision; this reads the result from outside.
2. **The deflate-the-hype mode** — "Beyond The Hype" + "AI didn't just make them redundant" is **narrative-deflation** content, the same turn [[nate-herk]] took in [[claude-mythos]]. The vault's two anchor creators are converging on skepticism as a content mode.
3. **A competitive-intel surface** — "read the cut to read the balance sheet" is a reusable analyst move for competitor teardowns; a layoff's shape is a free health/strategy signal.
4. **TalentBoard tie-in** — the "where job seekers should look" payoff feeds his fluency-signaling job board; framework-as-content-marketing, same pattern as his Substack-gated kits.

> ⚠️ The specific distress-detection rules and the Meta/Block balance-sheet evidence are **transcript-gated** (bio/description-only digest entry).

→ New concept: [[ai-layoffs]]. Updates: [[ai-supply-contract]] (CapEx-vs-headcount link), [[portable-judgment]] (labor-market companion to the career-evidence frame).

## New in [[youtube-digest-apify-2026-06-24]]

**Five videos in one batch — his 30th, 31st, and 32nd named frameworks plus two news reads — landing on a single throughline: ownership and accountability.** (Dated 06-19 → 06-23.)

### #7 [[open-skills]] — own portable procedures, not rented prompts (30th framework)

*The Skill vs Prompt Problem Everyone Gets Wrong* (24.8K views, 2026-06-19, 17:45) — and the launch of his **Open Skills** project. Agent skills **don't travel** between [[claude-code]], [[codex]], and Cursor — *"one of the most expensive problems in AI work."* **Memory alone doesn't fix agents** (the Open Brain problem); the unsolved part is *who owns the procedure when you switch tools.* Prompt bloat becomes **procedural debt** (four places it shows up); a **real skill ≠ a clever prompt** (it verifies + composes — *"skills as primitives, runbooks as compositions"*); **own portable procedures, not rented ones.** The cross-vendor portability sharpening of [[nate-herk]]'s 3-layer `CLAUDE.md`↔`AGENTS.md` model (Herk showed instructions-layer symmetry; Jones argues the *skill* layer is where portability breaks). → New concept: [[open-skills]]. Updates: [[claude-skills]], [[skill-systems]], [[portable-judgment]].

### #5 [[creator-trust-stack]] — "made with AI" is five questions (31st framework)

*You Can't Tell If I'm Real Anymore. And That's Now YouTube's Problem Too.* (21.6K views, 2026-06-20, 9:21). Voice cloning *"already passes in a half-watching feed"*; **full human presence breaks in the small details**; the **uncanny valley got relational** (the tell is interaction/continuity, not a frame). **"Made with AI" splits into five questions** — where AI helps vs where human judgment must own the result. The **creator trust stack**: *no never-use-AI rule*; winners **stay legibly human, disclose clearly, and keep a real person accountable for what ships.** The creator-side instantiation of [[prove-it-economy]]'s truth layer. → New concept: [[creator-trust-stack]]. Updates: [[prove-it-economy]], [[public-ai-work]].

### #12 [[agent-ownership]] — name an owner for every agent (32nd framework)

*You Can't Run AI Agents Without This* (18.6K views, 2026-06-21, 14:20). Once an agent reads real files and does real work, **somebody has to own it, and most people cannot say who.** A working agent **stops being a tool and becomes a responsibility** (assistant vs agent). **Care and feeding = job / diet / boundaries / review loop**; the artifacts are an **agent roster + owner card** (named human per agent). *"Maintenance is the 2026 skill."* The **accountability layer that completes his enterprise-AI agent stack** — [[agent-security]] governs each action, [[agent-analytics]] measures each run, [[infrastructure-control-layer]] holds the kill switch, and agent-ownership names the human. → New concept: [[agent-ownership]]. Updates: [[agent-security]], [[agent-analytics]], [[anticipation-gap]].

### #11 [[claude-fable-5]] — the doing got cheap (news/interpretation)

*The Doing Got Cheap. Now What? | Claude Fable 5 Changes Work* (30.4K views, 2026-06-23, 18:11). Fable 5 is *"the biggest model in the world"* but **the real story isn't the benchmarks** — the bottleneck moved from *what the model can do* to *what you can imagine handing it.* **Task imagination replaces prompt engineering** as the core skill; *"the doing is getting cheap and the deciding is not"*; **model managers** as the durable role. The successor to his own [[ai-question-method]] (prompting-obsolete) and the capability floor under [[software-abundance-pm]]. → New concept: [[claude-fable-5]].

### #4 *Google Lost $2.7 Billion In Talent This Week* (news/interpretation)

49.0K views, 2026-06-22, 8:22. OpenAI *looks* like it won the week (the **Shazeer/Noam-tier hire**, "5.6" rumors, Fable headlines), but Jones argues **[[anthropic]] may quietly lead on pre-training cadence**, Nobel-tier talent movement signals recursive self-improvement, and the biggest story may be **Midjourney's medical-imaging breakthrough** — outside either lab. *"Betting on one lab's headline week is fragile."* → Updates: [[anthropic]], [[openai]].

**Strategic significance**: three new frameworks in one batch ([[open-skills]] / [[creator-trust-stack]] / [[agent-ownership]]) all encode the same principle — **own the asset, name the responsible human**: own the *procedure* (portable skills), name the *agent's* owner, keep a real *person* accountable for what ships. It's the ownership/accountability counterpart to his earlier T/C/L/D (own your durable work) and [[portable-judgment]] (own your career evidence). **32 named frameworks in ~48 days** (T/C/L/D 2026-05-04 → agent-ownership 2026-06-21).

## New in [[youtube-digest-apify-2026-06-25]]

### #2 [[loop-of-loops]] — agents are loop managers (33rd framework)

*I Stopped Prompting AI One Task At A Time. This Works Better.* (22.2K views, 2026-06-24, 15:38). *"AI agents get useful when you stop prompting one task at a time and start building loops."* A **loop** = *"a recurring job that remembers, notices what changed, and stops where your judgment matters"*; a **loop of loops** composes sub-loops that **notice each other and hand off what changed** (the **school-trip** demo); **agents are "loop managers."** The **prompt → loop → loop of loops** ladder argues most people are stuck prompting one task at a time while the leverage is one or two rungs up; apps *"left the hard part to you"* (the recurring orchestration), which is why a week *"feels heavier than it should."* The guardrail: each loop needs **clear boundaries** and must **stop where your judgment still matters** — the recurring-job restatement of his [[anticipation-gap]] permission ladder. The **consumer/life-admin demand-side complement** to [[nate-herk]]'s same-week runtime [[agent-loops]] (Herk = how one loop runs; Jones = which loops to build + where they stop). Extends his 06-24 ownership/loops cluster ([[open-skills]] / [[agent-ownership]] / [[creator-trust-stack]]) with the structure-the-recurring-work layer. → New concept: [[loop-of-loops]]. Updates: [[agent-loops]], [[anticipation-gap]], [[agent-ownership]]. **33 named frameworks in ~52 days** (T/C/L/D 2026-05-04 → loop-of-loops 2026-06-24).

## New in [[youtube-digest-apify-2026-06-27]]

### #1 [[open-engine]] — the handoff is the bottleneck (34th framework)

*I Was The Only Thing Connecting Claude, ChatGPT, and Codex. So I Built My Replacement.* (17.9K views, 2026-06-26, 22:04). *"Your AI agents don't talk to each other, so you have become the human glue moving work between Claude, Codex, and ChatGPT."* The common story is agents go autonomous; the reality is **the handoff between agents, not the model, is the bottleneck** — context, sources, and review keep falling back on the human. **Open Engine** is a **shared task queue both people and agents read** that lets agents **hand off work, carry the sources, and leave a receipt** without a human in the middle. *"Agents are loop managers and you are the hallway"* (04:48); *"the fix is a shared queue both people and agents read"* (06:32). The behavioral ask: move from **prompt mode** to **work mode** — *"the agents are finally capable enough to do the work, and the win now is building the queue that moves it between them before you become the bottleneck."*

This is his **third consecutive cross-vendor-coordination framework** ([[open-skills]] 30th → [[loop-of-loops]] 33rd → open-engine 34th), all circling the same insight: in a multi-tool 2026 workflow the scarce missing piece is the **layer between agents**, not the agents. Open Engine is the **build-side instantiation** of [[agent-protocol-stack]]'s A2A (agent-to-agent delegation), the **work-side cousin** of [[open-skills]] (procedures don't travel ⇄ *work* doesn't travel), and the **multi-agent extension** of [[loop-of-loops]] (loops handing off across *different agents*). → New concept: [[open-engine]]. Updates: [[agent-protocol-stack]], [[open-skills]], [[loop-of-loops]], [[work-primitive]], [[anticipation-gap]]. **34 named frameworks in ~54 days** (T/C/L/D 2026-05-04 → open-engine 2026-06-26).

## New in [[youtube-digest-apify-2026-06-30]]

**Two videos in one batch — both fronts of the same context-as-moat argument.**

### #4 [[glm]] and the last mile — *why can't companies switch?*

*GLM 5.2 Is Free And Beats Claude On Most Work. So Why Can't Companies Switch?* (83.5K views, 2026-06-28, 17:35). [[glm]] 5.2 is a **free, open-source model that often beats [[opus-4-8|Claude]] on everyday work**, yet companies keep paying frontier prices. Thesis: *"the real bottleneck is no longer the model call. It is the **last mile** around it: context, routing, and harnesses."* Key moves:

- **Center-of-distribution vs edge-of-distribution tasks** (4:11) — GLM can safely replace a frontier model on common work; the edge still rewards frontier.
- **Switching a model = replacing a whole work system, not a call** — *"the real question is whether you can move your context, not whether the model can answer your prompt."*
- **[[claude-tag]] turns your team's Slack context into a sticky harness** — the named mechanism that holds customers as intelligence commoditizes ~98% cheaper.
...
The **demand-side / lock-in explanation** for why [[glm]]'s ~5× price advantage (Nate Herk's 06-19 video) hasn't triggered mass switching — confirms [[harness-over-model]] from the enterprise-adoption angle, and the first vault surfacing of **[[claude-tag]]**. → Updates: [[glm]], [[harness-over-model]]. New concept: [[claude-tag]].

### #5 [[context-wars]] — from the intelligence wars to the context wars

*The Real Story Behind the Government GPT 5.6 Freeze* (32.1K views, 2026-06-29, 17:13). → New concept: [[context-wars]]. *"The real story isn't which lab has the smartest model. It's who controls the context that makes any model useful: your files, your Slack, your phone."* As frontier intelligence (and frontier *releases* — the government GPT-5.6 freeze) slows, **context becomes the next real advantage.** Three fronts (chapter map): **Apple → Siri/phone context** (3:13); **Anthropic → [[claude-tag]] inside Slack** (5:44); **OpenAI → [[codex]] earning trust on sensitive work** (8:29); against the backdrop of **the US government slowing frontier releases** (12:51). Privacy hinge: *"the same access quietly turns every convenience into a decision about what you are willing to hand over."*

The **macro-thesis umbrella** over the vault's context cluster — [[harness-over-model]], [[glm]], [[open-engine]], [[karpathy-llm-wiki]] (data moat), [[ai-operating-system]] ("context is king") are all moves inside the context wars; #4 and #5 are the **enterprise-switching-cost** and **industry-structure** statements of the same thesis. **His 35th tracked framing in ~56 days.** → New concept: [[context-wars]]. Updates: [[claude-tag]], [[anthropic]] (Claude Tag launch), [[codex]] (trust-adoption datapoint).

## New in [[youtube-digest-apify-2026-07-02]]

**One video — *own the memory, rent the intelligence*: the [[context-wars]] thesis applied to personal AI memory.**

### #1 Build your own AI memory by talking to Claude

*I Built My Own AI Memory by Talking to Claude. It Did 80% Itself.* (26.5K views, 2026-07-01, 16:16). The framing claim: *"the common story is that you wait for the next assistant from a big lab, but the real move is **owning the memory yourself and renting the intelligence**."* Agents are finally good enough to **build ~80% of your personal AI-memory stack for you, just by talking to Claude or [[codex]]** — a personal agent that *"starts from your context, follows your intent, and waits for your yes before it acts."*

The four moves (chapters):
- **Build 80% of the stack by talking to your agent** (04:01) — the agent scaffolds its own memory system; you supply intent + approvals.
- **Owning your memory > renting intelligence** (04:37) — his [[context-wars]] / [[karpathy-llm-wiki]] "data moat" thesis at the **personal** scale: memory is the durable layer, the model is the rented commodity.
- **Boundaries + the intent loop** (02:07, *why intent became the central problem*) — the agent *waits for your yes before it acts*; the permission-ladder / action-boundary discipline ([[anticipation-gap]], [[agent-security]], [[agent-ownership]]).
- **Start with one repeated part of your life** (05:25) — the wedge: make one workflow predictable enough to hand over (his standing [[anticipation-gap]] prescription).

Chapters also name **"Open Engine and orch"** (06:15) and open on *"what Nikita's agent did to Lemonade"* (an insurance-agent story) — placing this as the **personal-memory-stack build underneath his [[open-engine]] coordination layer**: Open Engine moves *work* between agents; this video builds the *memory* each agent starts from. Same-batch resonance: [[cole-medin]]'s [[open-knowledge-format]] (#2 in this digest) is the **portability standard** that makes owned memory travel across any rented intelligence — Jones supplies the *ownership* thesis, OKF supplies the *portable format*.

→ Updates: [[open-engine]] (memory stack beneath the coordination layer), [[knowledge-layer]] (own-your-memory / personal data-moat), [[agent-ownership]] (intent loop + waits-for-your-yes boundaries), [[context-wars]] (personal-scale instance). His 36th tracked framing in the vault.

## New in [[youtube-digest-apify-2026-07-03]]

**One video — a practical model-picker: *route by the job, not the model*.**

### #1 [[model-routing]] — start with the job, not the model

*Stop Wasting Money on the Wrong AI* (11.1K views, 2026-07-02, 14:17). → **New concept: [[model-routing]].** The framing: *"every AI model suddenly looks replaceable, and picking the right one has turned into a second job."* His reframe: *"the common story is that the smartest model wins; the real question is which intelligence a specific job actually needs."*

The routing rules (chapter map):
- **Start with the job, not the model** (01:42) — route by the *shape of the work*, not the leaderboard.
- **Cheap workhorse for familiar, repeatable work** (02:28) — send known work to a cheaper model like [[glm|GLM 5.2]] and *review it fast*.
- **Frontier model when the shape of the job is unclear** (03:25) — plus *"[[claude-fable-5|Fable]]-style problems that need the strongest model"* (04:47).
- **Daily driver + why the harness matters** (03:53) — the daily driver is a harness decision, a direct callback to [[harness-over-model]].
- **Specialists win** at images, video, live web, and coding harnesses.
...
This is the **prescriptive, per-job model-picker** that operationalizes his earlier [[harness-over-model]] thesis (the score doesn't make a daily driver) into an actual decision matrix, and restates the *"keep your context portable"* close as an operator habit inside [[context-wars]]. Test-on-your-own-work posture ("test any model on your own w[ork]", 05:40) matches [[long-running-benchmarks]]. → New concept: [[model-routing]]. Updates: [[glm]] (canonical cheap-workhorse role), [[harness-over-model]] (the picker for the thesis), [[context-wars]] (portable-context habit), [[claude-fable-5]] (strongest-model tier). His 35th named framework (37th tracked framing).

## New in [[youtube-digest-apify-2026-07-04]]

**One video — *build one reusable agent, point it at everything*: the [[reusable-agent-skeleton]], his 36th named framework.**

### #1 [[reusable-agent-skeleton]] — every AI agent demo stops at email; I pointed mine at the bills

*Every AI Agent Demo Stops at Email. I Pointed Mine at the Bills That Cost You Money.* (11.6K views, 2026-07-03, 15:44). → **New concept: [[reusable-agent-skeleton]].** Framing: *"AI agents usually get rebuilt from scratch for every new job. Here's how to build **one reusable AI agent** for messy, high-trust paperwork — insurance appeals, tax prep, and beyond… the real question is **what you build once and point at everything else**."*

The framework (chapters):
- **Email/calendar is the 101 where mistakes stay cheap** (cold open, 0:00) — learn the pattern on low-stakes work before the stakes rise.
- **One nine-step skeleton carries unchanged across three builds** (02:25) — email/calendar → **denied insurance claim** (06:55) → **tax-prep packet** (10:49); *build once, point at everything*, the opposite of rebuild-per-job.
- **A cited appeal packet must do specific things and never promise** — the [[document-truth-layer]] discipline (evidence + citations, no overclaiming) on a regulated artifact.
- **The human-approval gate stays locked at the end of every build** (12:27, *three builds, same gate*) — *"as long as the last decision stays yours"*; the [[agent-security]] / [[agent-ownership]] permission boundary as the non-negotiable payoff.
- **Clean data + model choice** (13:09) — data-quality precondition + a callback to his [[model-routing]] picker.

Strategic read: this **operationalizes several standing frameworks at once** — the reusable *skeleton* is [[open-skills]]' "portable procedure" made concrete; the *gate* is [[agent-security]] / [[agent-ownership]]; the *cited packet* is [[document-truth-layer]]; *clean data + model choice* is [[model-routing]]. The *learn-on-cheap-email-then-point-at-expensive-paperwork* wedge is his [[anticipation-gap]] "start with one repeated part of your life" prescription applied to high-trust, regulated work. Continues the same *"durable spine, portable pieces, human keeps authority"* thread as last batch's *own-your-memory* (2026-07-02) and *model-routing* (2026-07-03).

→ Updates: [[agent-security]] (locked approval gate), [[agent-ownership]] (last decision stays yours), [[document-truth-layer]] (cited, never-promise packet), [[open-skills]] (portable procedure), [[model-routing]] (model-choice lever). His **36th named framework** (38th tracked framing).

## New in [[youtube-digest-apify-2026-07-05]]

**One video — a short (3:50) Fable 5 operator tip: the *goal-harness* move.** Not a numbered framework; a tactical read that extends [[harness-over-model]].

### #1 *Free Fable 5 tokens this weekend? Here's how to max them*

*Free Fable 5 tokens this weekend? Here's how to max them* (16.8K views, 2026-07-04, 3:50). Timed to a **free-token holiday weekend** — *"Fable 5 is the AI model everyone's racing to max out this holiday weekend, but most people are pointing it at the wrong work."* The load-bearing correction:
...
Four tips (from the description):
- **Wire Fable 5 into tools like Blender for real output** — the frontier model driving an external tool, not just chatting.
- **Short prompts beat long ones on hard problems** — the counter-intuitive prompting posture for the strongest model (his standing *AI-as-senior-partner* [[ai-question-method]] line pushed to Fable).
- **What business problems are worth handing to it directly** — Fable for the horsepower-needing minority, per [[model-routing]].
...
Strategic read: this **extends [[harness-over-model]] one step** — that framework said *the harness beats the raw score*; here the frontier model's best use is to **build the harness itself** (the harness-author role, above planner and coder). The *"aim it at problems that need the horsepower"* close restates [[model-routing]]; the *"won't stay free"* framing is a [[free-sample-phase]] datapoint. Same *durable-spine, portable pieces* thread as the prior batch's [[reusable-agent-skeleton]] (2026-07-04). → Updates: [[claude-fable-5]] (goal-harness section), [[harness-over-model]], [[free-sample-phase]].

## New in [[youtube-digest-apify-2026-07-06]]

**One video — the 37th tracked framework: [[imagination-and-permission]].** The **demand-side companion** to his own [[model-routing]] picker three days earlier.

### #1 [[imagination-and-permission]] — you can't compete on cheap models anymore (37th framework)

*You Can't Compete on Cheap Models Anymore* (43.9K views, 2026-07-05, 15:39). The framing: routing execution to cheap models is *"the right call and about to be table stakes, which means the real advantage is moving to a layer almost nobody is budgeting for."* The reframe:
...
Evidence beats: the **$40 test** — *"a $1 model now ties a $9 model on routine work"* (execution commoditizes); *"the test the cheap models couldn't touch"* → **cheap engine, frontier steering** (a [[harness-over-model]] / goal-harness restatement); **BlackBerry vs Apple** — equal execution, imagination as the multiplier. Punchline: *"if your task list hasn't changed in a year, your constraint on AI returns is imagination and permission, not price."*

Strategic read: this **fuses [[claude-fable-5|task imagination]] with [[model-routing]]**. Because everyone will route to cheap models, routing stops being where returns come from, so the binding constraint snaps back to the demand side — **what work you can imagine and are permitted to build.** *"Permission"* is the newly explicit second half: the organizational freedom to change a task list, tying into [[public-ai-work]] + [[agent-ownership]]. → Updates: new [[imagination-and-permission]]; [[model-routing]] (demand-side companion), [[harness-over-model]] (cheap-engine-frontier-steering), [[claude-fable-5]].

## New in [[youtube-digest-apify-2026-07-07]]

**One video — the new AI scoreboard: compute + distribution + government permission.** The industry-structure statement of his [[context-wars]] thesis.

### #1 OpenAI's $42B government offer — the scoreboard changed

*OpenAI Just Offered The Government $42 Billion. This Is The Real Reason.* (13.9K views, 2026-07-06, 12:36). The framing: *"Meta, OpenAI, and Anthropic all made moves this week that reveal how the AI strategy game is actually changing. The common scoreboard — who has the best model — is being replaced by a fight over **compute, distribution, and government permission**."*

Four moves as evidence (chapters):
- **Meta's consumer gaming app + a new AI-compute-selling business are one strategy** (01:59, 02:27) — Bloomberg reports Meta standing up a business to *sell access to its AI compute*; the gaming app and the cloud business are two faces of the same distribution/compute play.
- **OpenAI's proposed 5% government stake** — the *"$42 billion"* headline is the **government-permission** front: buying political permission, not just capability.
- **Jersey Mike's AI-heavy IPO filing** (07:10) — *"how bad the AI hype has become"*; cheap capital flooding anything AI-adjacent.
- **Anthropic's enterprise bet** — where it fits in the new scoreboard.

Punchline: *"AI capability keeps compounding no matter who wins the model race, but the risk is building your strategy around a scoreboard that no longer decides who actually captures the value."*

Strategic read: this is the **supply-and-permission-side mirror** of [[context-wars]] — where that thesis named the demand-side moat (who controls your files/Slack/phone), this names the structural moat (**compute + distribution + government permission**). The builder-side companions already in the vault are [[ai-supply-contract]] (HBM/packaging/power bottleneck) and [[infrastructure-control-layer]] (the 5 substrate control points). Same-batch resonance: [[nate-herk]]'s *"the model isn't the moat"* ([[fable-mode-skill]], #4 this digest) is the operator-scale version of Jones's industry-scale claim. → Updates: [[context-wars]].
...
- [[agent-substrate]], [[agentic-commerce]], [[anticipation-gap]], [[work-primitive]], [[plugins]], [[code-comprehensibility]], [[agent-security]], [[retrieval-contract]], [[agentic-implementation-layer]], [[agent-metering]], [[capital-allocation-framework]], [[agent-protocol-stack]], [[prove-it-economy]], [[ai-question-method]], [[infrastructure-control-layer]], [[project-room-workflow]], [[long-running-benchmarks]], [[ai-supply-contract]], [[platform-agent-asymmetry]], [[public-ai-work]], [[document-truth-layer]], [[agent-analytics]], [[software-abundance-pm]], [[portable-judgment]], [[harness-over-model]], [[token-burn-dashboard]], [[ai-layoffs]], [[open-skills]], [[creator-trust-stack]], [[agent-ownership]], [[claude-fable-5]], [[loop-of-loops]], [[open-engine]], [[model-routing]], [[reusable-agent-skeleton]], [[imagination-and-permission]] — concepts he originated/popularized
- [[agent-loops]] — [[nate-herk]]'s runtime loop; the how-one-loop-runs half to his demand-side [[loop-of-loops]]
...
- [[youtube-digest-apify-2026-05-28]] — **2 frameworks (22nd + 23rd)**: [[public-ai-work]] (Shopify River agent / apprenticeship gap / Polanyi's paradox) + [[document-truth-layer]] (workflow-defines-trust / four-stage pipeline / hostile-reviewer prompt / task risk gradient)
- [[youtube-digest-apify-2026-05-29]] — **24th framework**: [[agent-analytics]] (run-not-click product analytics / completion-vs-acceptance gap / the-correction-is-your-most-valuable-signal / Salesforce Agent Work Units / Cursor-DB-wipe unlock)
- [[youtube-digest-apify-2026-05-30]] — **25th framework**: [[software-abundance-pm]] (PM classifies software abundance into market value / artifact-arrives-before-the-request / non-technical-PM-is-finished / Microsoft 1M+ Power Platform assets / production-class-ladder / demotion-matters-as-much-as-promotion)
- [[youtube-digest-apify-2026-06-02]] — **26th framework + practice-update**: [[portable-judgment]] (86%-treat-AI-output-as-starting-point / age-of-whiteboards / four-questions / comprehension-over-generation / preserve-evidence-before-badge-stops-working) + *My AI Workflow Has Changed* (shape-the-task / local-folders-as-context-windows / why-not-pick-a-side)
- [[youtube-digest-apify-2026-06-05]] — **27th framework**: [[harness-over-model]] (Opus 4.8 = checkpoint release / harness > benchmark score / reasoning-effort unpredictable / Vending-Bench effort-level trap / Codex harness beat the score / routing guide) — first tracked creator disagreement with [[nate-herk]] on a Claude model
- [[youtube-digest-apify-2026-06-06]] — **28th framework**: [[token-burn-dashboard]] (token-burn-tracks-with-smarter-results / the-point-is-the-feedback-loop / built-in-Codex / multi-agent-runs-reveal-real-habits / assistant-work-vs-computer-work / ranking-by-volume-backfires / Tufte-viz + 800M-tokens) — operator-side cousin of [[agent-analytics]]
- [[youtube-digest-apify-2026-06-09]] — **29th framework**: [[ai-layoffs]] (no-two-layoffs-mean-the-same-thing / a-layoff-is-a-strategy-signal / Meta-cuts-while-burning-GPU-billions = reallocation / Block-encodes-AI-vision / activity-based-cuts-reveal-hidden-distress / job-seeker-run-look-map) — labor-market companion to T/C/L/D + [[portable-judgment]]
- [[youtube-digest-apify-2026-06-24]] — **3 frameworks (30th–32nd)**: [[open-skills]] (skills-don't-travel / procedural-debt / own-portable-procedures) + [[creator-trust-stack]] ("made with AI" is five questions / legibly-human + disclose + accountable) + [[agent-ownership]] (owner card / job-diet-boundaries-review / "maintenance is the 2026 skill") — plus [[claude-fable-5]] (task-imagination-replaces-prompting) + the $2.7B-talent news read
- [[youtube-digest-apify-2026-06-25]] — **33rd framework**: [[loop-of-loops]] (agents are loop managers / prompt → loop → loop-of-loops ladder / stop where your judgment matters)
- [[youtube-digest-apify-2026-06-27]] — **34th framework**: [[open-engine]] (the handoff is the bottleneck / human glue / shared task queue both people and agents read / hand off work + carry sources + leave a receipt / prompt mode → work mode)
- [[youtube-digest-apify-2026-07-03]] — **35th named framework**: [[model-routing]] (start with the job, not the model / cheap workhorse for familiar work / frontier for unclear-shape jobs / [[claude-fable-5|Fable]] for the strongest problems / specialists for images-video-web-code / keep your context portable) — the prescriptive per-job picker for [[harness-over-model]]
- [[youtube-digest-apify-2026-07-04]] — **36th named framework**: [[reusable-agent-skeleton]] (one nine-step skeleton / learn on email/calendar 101 / point at insurance appeals + tax prep / cited-packet-never-promise / human-approval gate stays locked / clean data + model choice) — build-once operationalizing [[open-skills]] + [[agent-security]] + [[document-truth-layer]] + [[model-routing]]
- [[youtube-digest-apify-2026-07-05]] — Fable 5 operator tip (not a numbered framework): the **goal-harness** move — use [[claude-fable-5|Fable 5]] to *design the goal harness that steers whatever coding model you hand the work to*; short prompts beat long on hard problems; wire into Blender; aim it only at horsepower-needing work — extends [[harness-over-model]] + a [[free-sample-phase]] datapoint
- [[youtube-digest-apify-2026-07-06]] — **37th named framework**: [[imagination-and-permission]] (routing to cheap models is table stakes / a $1 model ties a $9 model on routine work / cheap engine, frontier steering / BlackBerry-vs-Apple imagination multiplier / "if your task list hasn't changed in a year, your constraint is imagination and permission, not price") — the demand-side companion to [[model-routing]]
- [[youtube-digest-apify-2026-07-07]] — news/interpretation (not a numbered framework): OpenAI's $42B government offer — the AI scoreboard shifts from *best model* to **compute + distribution + government permission** (Meta selling compute access / OpenAI 5% government stake / Jersey Mike's AI-heavy IPO / Anthropic's enterprise bet) — the supply-and-permission-side statement of [[context-wars]]
