...
- **[[claude-subagents|Subagents]]** (rung 2 of the orchestration ladder; given dedicated coverage 2026-06-09 via [[nate-herk]] in [[youtube-digest-apify-2026-06-09]]) — **scoped delegate agents with their own context window**; keep the main context clean (the mess stays in the subagent's window) and **save money** (run read-only / on a cheaper model, reserve [[opus-4-8]] for orchestration). Auto-invoked off their **description** (progressive disclosure, like [[claude-skills]]); built-in vs custom; project vs global scope. The delegation unit that agent teams coordinate and dynamic workflows script. → See [[claude-subagents]]
...
## Patterns added in [[youtube-digest-apify-2026-06-09]]

- **Subagents get their own explainer — rung 2 unpacked** ([[nate-herk]] #1, 14.5K views) — *How to Build Claude Subagents Better Than 99% of People.* After unpacking the *top* of the complexity ladder ([[dynamic-workflows]]), Nate isolates **rung 2**: a subagent is a **scoped delegate with its own context window** that *"keeps your main context clean"* and *"saves you money"* (cheaper model + read-only scope while one smart model orchestrates). Auto-invoked off its **description** (progressive disclosure); built-in vs custom; **Skills vs Subagents** = knowledge unit vs worker unit; project vs global scope. → New concept: [[claude-subagents]].
- **The 712.5K-view Build & Sell flagship** ([[nate-herk]] #4, back-catalog 2026-03-12, 10:00:05) — *Build & Sell with Claude Code (10+ Hour Course)* — his **highest-view video in the vault**, a no-code zero-to-income course (*"we don't ever write a single line of code"*): setup → tokens/context → Claude.md → workflows → deploy → RAG → APIs/MCPs → finding clients → pricing. Confirms long-form "learn Claude Code + make money" is his highest-reach format. → See [[ai-consulting]].
- **Graphify — a repo knowledge graph to cut token cost** ([[jack-roberts]] #5, 23.4K views) — *Claude Code + Graphify = Insane Agentic OS.* Builds a **knowledge graph of any repo** so Claude *"reads the map instead of skimming the whole repo every single session"* — cheaper, faster, fewer wasted tokens. Plugged into a [[hermes-agent|Hermes]]-based cross-device agentic OS ("one shared brain across Claude Code, laptop, mobile"); first vault mention of **AntiGravity**. The codebase-scale instantiation of [[retrieval-contract]]. → New concept: [[graphify]].
...
- [[claude-subagents]] — rung 2 of the orchestration ladder, given its own explainer ([[nate-herk]] 2026-06-09)
- [[graphify]] — repo knowledge graph to cut token cost ([[jack-roberts]] 2026-06-08)
- [[agent-council]] — anti-yes-man idea-validation panel run inside the build loop; long sessions degrade, so reset context without losing your place + find the throughput bottleneck ([[nate-herk]] 2026-06-25)
...
- [[youtube-digest-apify-2026-06-09]] — subagents + Build & Sell course + Graphify
...
- [[watch-skill]] — the `/watch` skill closes Claude Code's video-comprehension gap ([[sources/youtube-digest-apify-2026-07-03]])
